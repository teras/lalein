...
An example of this file can be found in file [Localizable.json](json/src/test/resources/Localizable.json) in JSON format
or [Localizable.yaml](yaml/src/test/resources/Localizable.yaml) in YAML format. To use in your own programs you need
...
`lalein` comes to fill the gap; a lightweight Java library with only 11K of core code, plus 7K for JSON (or 7K for YAML)
support (and the size of the JSON/YAML library itself). When using `lalein` messages with arbitrary complexity,
like
...
are easy to implement. The configuration is in plain, human-readable, JSON (or YAML) format. And also, a GUI frontend is
provided, to visually test translation files.
...
#### Using JSON as a backend format
...
scenarios using the JSON backend.
...
as with the JSON format apply. Only the syntax of the configuration file changes.
